...
GPUMatrix::GPUMatrix(size_t rows, size_t cols): ptr{cuda::mallocRaw<float>(rows * cols)}, rows{rows}, cols{cols}, should_delete{true} {}
...
GPUMatrix::GPUMatrix(const GPUMatrix& other): ptr{other.ptr}, rows{other.rows}, cols{other.cols}, should_delete{false}{}
...
GPUMatrix GPUMatrix::Identity(size_t n)
{
    GPUMatrix res(n, n);
...
GPUMatrix GPUMatrix::from_vector(const std::vector<Eigen::Vector3f>& p)
{
    auto matrix = GPUMatrix(p.size(), 3);

    cudaMemcpy(matrix.ptr, p.data(), p.size() * sizeof(Eigen::Vector3f), cudaMemcpyHostToDevice);

    return matrix;
}

std::vector<Eigen::Vector3f> GPUMatrix::to_vector() const
{
    assert(cols == 3);
    std::vector<Eigen::Vector3f> list(rows);

    cudaMemcpy(list.data(), ptr, rows * sizeof(Eigen::Vector3f), cudaMemcpyDeviceToHost);

    return list;
}

float3* GPUMatrix::to_float3(size_t length) const
{
    float3* d_points;
    cudaMallocManaged((char **)&d_points, length * sizeof(*d_points));
    cudaMemcpy(d_points, ptr, length * sizeof(*d_points), cudaMemcpyDeviceToDevice);
    return d_points;
}

GPUMatrix GPUMatrix::from_cpu(const Eigen::MatrixXf& cpu)
{
    GPUMatrix res(cpu.rows(), cpu.cols());
    cudaMemcpy(res.ptr, cpu.data(), cpu.cols() * cpu.rows() * sizeof(float), cudaMemcpyHostToDevice);
    return res;
}

Eigen::MatrixXf GPUMatrix::to_cpu() const
{
    Eigen::MatrixXf res(rows, cols);
    cudaMemcpy(res.data(), ptr, cols * rows * sizeof(float),cudaMemcpyDeviceToHost);
    return res;
}
...
__device__ void block_sum_unroll(volatile float* memory, int tidx)
{
...
__global__ void block_sum_kernel(GPUMatrix inputs, GPUMatrix res)
{
    ...
    int line = blockIdx.x * blockDim.x * 2 + threadIdx.x;
    ...
    for (int s = blockDim.x / 2; s > 32; s >>= 1u)
    {
        if (threadIdx.x < s)
...
GPUMatrix GPUMatrix::sum_colwise() const
{
    ...
    constexpr int block_widths[] = {0,  1,  2,  4,  4,  8,  8,  8, 8, 16, 16, 16, 16, 16, 16, 16, 16};
    int block_width = block_widths[cols];

    dim3 blockdim_block_sum(1024 / block_width, block_width);
    blockdim_block_sum.x *= 2;
    dim3 griddim_block_sum((rows + blockdim_block_sum.x - 1) / blockdim_block_sum.x, (cols + blockdim_block_sum.y - 1) / blockdim_block_sum.y);
    blockdim_block_sum.x /= 2;
    auto sums = GPUMatrix::zero(griddim_block_sum.x, cols);
    block_sum_kernel<<<griddim_block_sum, blockdim_block_sum, blockdim_block_sum.x * cols * sizeof(float)>>>(*this,sums);
...
GPUMatrix GPUMatrix::mean() const
{
    ...
    dim3 griddim_divide((rows + blockdim_divide.x - 1) / blockdim_divide.x, (cols + blockdim_divide.y - 1) / blockdim_divide.y);
    divide_kernel<<<griddim_divide, blockdim_divide>>>(*this, divided);

    return divided.sum_colwise();
}

__global__ void subtract_rowwise_kernel(GPUMatrix a, GPUMatrix b, GPUMatrix res)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= a.rows)
        return;
...
GPUMatrix GPUMatrix::dot(const GPUMatrix& matrix) const
{
    ...
    dim3 blockdim(32, 32);
    dim3 griddim((res.rows + blockdim.x - 1) / blockdim.x,(res.cols + blockdim.y - 1) / blockdim.y);
...
__global__ void closest_kernel(GPUMatrix from, GPUMatrix to, GPUMatrix res)
{
    ...
    size_t closest_i = 0;
    float closest_dist = GPUMatrix::squared_distance(from, row, to, closest_i);
...
__global__ void product_kernel(GPUMatrix a, GPUMatrix b, GPUMatrix res)
{
    ...
    int i_a = blockIdx.y * blockDim.y + threadIdx.y;
    if (i_a >= a.cols)
        return;

    int i_b = blockIdx.z * blockDim.z + threadIdx.z;
    if (i_b >= b.cols)
        return;

    int i = i_a * a.cols + i_b;
